#!/usr/bin/env python3
"""
google-maps-scraper-api - CoreClaw Google Maps API Demo

This script demonstrates how to use CoreClaw's Web Data API for Google Maps data extraction.
...
class GoogleMapsScraperApi:
    """CoreClaw Google Maps API client demo."""
    ...
    def search(self, query: str, limit: int = 100) -> List[CoreClawResult]:
        """Search Google Maps data via CoreClaw API."""
...
def main():
    parser = argparse.ArgumentParser(description="google-maps-scraper-api - CoreClaw Google Maps API Demo")
    ...
    client = GoogleMapsScraperApi(api_key=args.api_key)
    client.search(args.query, args.limit)
    client.export(args.output, args.format)
    print(f"Done! Got {len(client.results)} records from CoreClaw Google Maps API.")
